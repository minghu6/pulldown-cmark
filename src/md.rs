...
use crate::{Alignment as TableAlignment, Event, HeadingLevel, LinkType};
...
///
/// 1. **events**
///   * An iterator over [`Events`][Event], for example as returned by the [`Parser`][crate::Parser]
/// 1. **formatter**
    ...
    F: fmt::Write,
{
    ...
    for event in events {
        use crate::{CodeBlockKind, Event::*, Tag::*};
...
pub fn push_md<'a, I, F>(events: I, mut formatter: F) -> Result<State<'static>, fmt::Error>
where
    I: Iterator<Item = Event<'a>>,
    F: fmt::Write,
{
    push_md_ext(events, &mut formatter, Default::default())
}
